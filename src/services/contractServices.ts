...
import { genrateSalt } from "@/utils/generateSalt";

let RpsContractAddress: `0x${string}`;
let stake = "";
let player1Move = 0,
  player2Move = 0;
...
export const startGame = async (
  ...
  try {
    stake = _stake;
    player1Move = move;
    const HasherContractAddress = "0xBd31D1C2C2a9E0259D305CF61880d0cA1a218712";
    ...
    RpsContractAddress = contract.target.toString() as `0x${string}`;
    lastAction = new Date();
  } catch (error) {
...
export const solveGame = async () => {
  try {
    ...
    const txnRes = await rpsContract.solve(player1Move, salt);
